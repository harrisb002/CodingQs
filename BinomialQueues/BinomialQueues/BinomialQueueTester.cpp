...
void testBinomialQueue(BinomialQueue &queue, int numTestsToRun) {
    ...
    while ( numTestFinished < numTestsToRun ) {
        ...
        if (action == INSERT) {
            ...
            if (queue.empty())
                ...
                std::cout << "Action is DELETE_MIN: min value to be deleted is: " << queue.findMin() << std::endl;
                ...
                smallIntInvertedList.remove(queue.findMin());
                queue.deleteMin();
            }
            ...
            if (queue.empty()) {
                ...
                if( queue.findMin() != smallIntInvertedList.minValue()) {
                    std::cout << "Smallest values in the queue is " << queue.findMin() <<
                              ". But, the smallest value in the inverted list is " << smallIntInvertedList.minValue() <<
                              "This is a bug.\n";
                    exit(1);
                }
                std::cout << "Action is FIND_MIN: The min value is: " << queue.findMin() << std::endl;
            }
        }
        numTestFinished++;
    }
    ...
    if( queue.empty()) {
        ...
        while(! queue.empty() ) {
            std::cout << queue.findMin() << std::endl;
            queue.deleteMin();
        }
