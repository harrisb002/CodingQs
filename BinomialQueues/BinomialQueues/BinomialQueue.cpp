...
BQnode *joinTrees(BQnode *tree1, BQnode *tree2) {
...
void BinomialQueue::insertTreeAtIndex(BQnode *treeToInsert, int idx) {
    ...
    for( int i = idx; i < heapOrderedTrees.size(); i++ ) {
...
int BinomialQueue::size() {
    ...
    int numNodes = 0;
    int treeSize = 1;
    for( int i = 0; i < heapOrderedTrees.size(); i++ ) {
        if( heapOrderedTrees.at(i) != nullptr )
            numNodes += treeSize;
        treeSize *= 2;
    }
...
int BinomialQueue::assertSizeOfTree(BQnode *tree) {
    if (tree == nullptr || tree->children().size() == 0)
...
void BinomialQueue::deleteMin() {
    ...
    if( minTreeIdx == heapOrderedTrees.size()) {
        // This is unexpected as minTree should not be null and
